...
Exp-03은 MoFlow 기반 ETH/UCY 보행자 궤적 예측 환경에서 Y-Flow를 평가한다. 기존 Exp-01
구현과 모델은 수정하지 않고 `experiments/exp_03_pedestrian/`에 격리한다.
...
Pedestrian Exp-03은 별도 의존성 환경에서 실행한다.

```bash
COMMAND=eval ./run_exp_03_pedestrian.sh --subset eth ...
```
...
├── datasets/pedestrian/default/   # ETH/UCY leave-one-out pkl splits
├── experiments/exp_03_pedestrian/ # isolated MoFlow-based implementation
├── data/
├── model/
├── train/
├── sample/
...
└── runs/exp_03_pedestrian/         # Exp-03 reports and raw artifacts
```
