...
#### hardflow
*   **설명**: `t_on`, `lambda_oc`, `max_iter`. training-free 샘플링만 사용 (PyTorch GPU-batched PGD 최적화).

#### yflow
*   **설명**: `t_on`, `lambda_oc`, `mu`, `max_iter`, `delta`, `gamma_max`, `safety_buffer`. 물리 연산자 $P$ warm start, PyTorch GPU-batched PGD 최적화, 립시츠 게이팅 기반 적응형 가이던스 $\gamma(t, \widehat{L}_P)$ 및 선형 보간을 사용.
...
#### uniconflow
*   **설명**: `free_until`, `ptzf_rate`, `gamma`, `slack_weight`, `max_guidance_norm`, `terminal_refinement`, `safety_buffer`. PTZF 기반 시간 지정 제약 certificate와 batched slack-QP 가이던스, 최종 매니폴드 투영(terminal refinement)을 사용.
